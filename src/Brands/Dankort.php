...
/**
 * Dankort is a Danish credit card brand that is widely used in Denmark.
 * It is issued by several Danish banks and is accepted at many merchants in Denmark and internationally.
 *
 * @see https://en.wikipedia.org/wiki/Dankort
 * @see https://en.wikipedia.org/wiki/Payment_card_number#Issuer_identification_number_(IIN)
 */
final class Dankort extends AbstractCreditCardBrand
{
    /**
     * The unique identifier for the Dankort.
     * @var non-empty-lowercase-string
     */
    public const ID = 'dankort';
    ...
    public function isValid(string $pan): bool
    {
        ...
        if (!str_starts_with($pan, '5019') && !str_starts_with($pan, '4571')) {
            return false;
        }
        try {
